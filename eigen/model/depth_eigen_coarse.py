...
class Depth_Eigen_Coarse(nn.Module):
    def __init__(self, **cfg):
        super(Depth_Eigen_Coarse, self).__init__()
        ...
        self.convs = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=96, kernel_size=11, padding=6, stride=4),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            nn.Conv2d(in_channels=96, out_channels=256, kernel_size=5, padding='same'),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            nn.Conv2d(in_channels=256, out_channels=384, kernel_size=3, padding='same'),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=384, out_channels=256, kernel_size=3, padding=1, stride=2),
            nn.ReLU(inplace=True)
        )
...
def depth_eigen_coarse(**kwargs):
    model = Depth_Eigen_Coarse(**kwargs)
    return model